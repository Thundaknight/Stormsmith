...
- **Discord bot** — each server posts as its own auto-updating message: an embedded card (color, uptime, players, …) with its control buttons directly beneath it, not bundled with other servers. Plus slash commands (`/servers`, `/server`, `/rcon`, `/broadcast`, `/wowcreate`, `/wowlevel`, `/wowgear`, `/wowrestock`), all gated by per-role feature permissions and per-command toggles configured in the web UI. Each server can display in its own channel, so you can run multiple status channels.
...
- Roles granted the **Manage AI bots** permission can run `/wowlevel @character <level>`, `/wowgear @character`, and `/wowrestock @character` (requires the `mod-playerbots` module and the Player database connection above) to manage a specific `mod-playerbots` AI bot by character name. All three look the character up in the Player database first and refuse to run on anything that isn't one of the server's bot accounts, so they can't be pointed at a real player — and the bot must currently be **online**, since AzerothCore's `.playerbots rndbot` commands only act on bots with a live in-world character; an offline (but otherwise valid) bot is rejected with a clear message instead of the confusing generic error AzerothCore itself would return.
  - `/wowlevel` raises a bot to a target level. AzerothCore's console only has a "raise by one level" command, not "set to level N", so Stormsmith calls it once per level needed to close the gap — it can't lower a bot's level.
  - `/wowgear` and `/wowrestock` both run `.playerbots rndbot refresh`, which repairs/re-equips gear *and* restocks ammo, food, reagents, consumables, and potions in one pass — AzerothCore doesn't expose those as separate console commands the way the in-game "autogear" and "maintenance" whispers do (those are chat-only and can't be invoked over SOAP at all), so both Stormsmith commands do the same thing under the hood; `/wowrestock`'s reply says as much.
  - None of the three get a confirmation message back from AzerothCore, so Stormsmith can only confirm the command was sent, not that it took effect. mod-playerbots also tracks which bots are *currently active* in an internal list separate from the database, which Stormsmith can't see — so even a bot that looks online in Player Accounts can occasionally still be rejected; if that happens, try again or pick a different bot.
  - All three can be restricted to specific Discord channels — see **AI bot command channels** on the Discord Bot page. When set, that list is authoritative for these commands (it replaces, not adds to, the bot's general command-channel restriction). Every use, including denied and failed attempts, is recorded on the **Logs** page with who ran it, when, and the result.
