...
- **Discord bot** — each server posts as its own auto-updating message: an embedded card (color, uptime, players, …) with its control buttons directly beneath it, not bundled with other servers. Plus slash commands (`/servers`, `/server`, `/rcon`, `/broadcast`, `/wowcreate`, `/wowlevel`, `/wowgear`), all gated by per-role feature permissions and per-command toggles configured in the web UI. Each server can display in its own channel, so you can run multiple status channels.
...
- Roles granted the **Manage AI bots** permission can run `/wowlevel @character` and `/wowgear @character` (requires the `mod-playerbots` module and the Player database connection above) to level up or re-gear a specific `mod-playerbots` AI bot by character name. Both commands look the character up in the Player database first and refuse to run on anything that isn't one of the server's bot accounts, so they can't be pointed at a real player. `/wowlevel` runs `.playerbots rndbot level`; `/wowgear` runs `.playerbots rndbot refresh`, which repairs and re-equips a bot at its current level — AzerothCore has no console command that upgrades gear quality the way the in-game "autogear" whisper does, so this is the closest available equivalent. Neither command gets a confirmation message back from AzerothCore, so Stormsmith can only confirm the command was sent, not that it took effect. These two commands can also be restricted to specific Discord channels, on top of the bot's general command-channel restriction — see **AI bot command channels** on the Discord Bot page. Every use (including denied and failed attempts) is recorded on the **Logs** page, with who ran it, when, and the result.
