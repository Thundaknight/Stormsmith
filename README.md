...
- **Discord bot** — each server posts as its own auto-updating message: an embedded card (color, uptime, players, …) with its control buttons directly beneath it, not bundled with other servers. Plus slash commands (`/servers`, `/server`, `/rcon`, `/broadcast`), all gated by per-role feature permissions and per-command toggles configured in the web UI. Each server can display in its own channel, so you can run multiple status channels.
