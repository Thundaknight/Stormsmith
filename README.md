...
- Roles granted the **Manage AI bots** permission can run `/wowlevel @character <level>`, `/wowgear @character`, and `/wowrestock @character` (requires the `mod-playerbots` module and the Player database connection above) to manage a specific `mod-playerbots` AI bot by character name. All three look the character up in the Player database first and refuse to run on anything that isn't one of the server's bot accounts, so they can't be pointed at a real player.
