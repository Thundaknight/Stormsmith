...
- **Mods tab** (admins only) — upload a BepInEx plugin DLL directly, or a Thunderstore `.zip` and Stormsmith unpacks it into place: a `BepInEx/` folder in the zip is merged into the container's BepInEx, a `plugins/` folder into `BepInEx/plugins`, and a bare DLL goes into `BepInEx/plugins/<PackageName>/`. The Thunderstore `manifest.json` / `icon.png` / `README.md` are dropped. BepInEx itself must already be enabled on your image. Restart the server to load the mods.
