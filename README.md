...
- Roles granted the **Create WoW accounts** permission (Discord Bot → Role permissions) can run `/wowcreate @user`, which DMs that person a "Set Password" button; submitting it creates their AzerothCore account over SOAP and confirms in the same DM (and to whoever ran the command, if different) once it's ready.
