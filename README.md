...
- **Discord bot** — auto-updating status message with a separate embedded card per server (each with its own color, uptime, players, and control buttons), slash commands (`/servers`, `/server`, `/rcon`, `/broadcast`), all gated by per-role feature permissions and per-command toggles configured in the web UI. Each server can display in its own channel, so you can run multiple status channels.
...
- A **Player Accounts** panel creates new login accounts (`.account create`, with an optional starting GM level) or resets an existing account's password (`.account set password`) — these are WoW login accounts for players, separate from Stormsmith's own web-interface users on the Users page.
